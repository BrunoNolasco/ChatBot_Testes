...
class TestObterResposta(unittest.TestCase):
    ...
    def teste_horas_e_data(self):
        ...
        resposta_horas = obter_resposta("que horas são")
        self.assertIn("São:", resposta_horas)
        self.assertIn(hora_atual[:4], resposta_horas)  # tolerância de minuto

        resposta_data = obter_resposta("qual é a data")
        self.assertIn("Hoje é dia:", resposta_data)
        self.assertIn(data_atual, resposta_data)

    def teste_resposta_padrao(self):
        """Teste de resposta padrão"""
        texto_aleatorio1 = "xyz123"
        texto_aleatorio2 = "blablabla"
        texto_aleatorio3 = "nada a ver"

        self.assertEqual(obter_resposta(texto_aleatorio1), f"Desculpa, não entendi a questão! {texto_aleatorio1}")
